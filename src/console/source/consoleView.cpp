...
namespace Labyrinth_44422 {
	namespace console {
		 ...
		 */
		ConsoleView::ConsoleView(const bool spacing, const bool arrows) : wall{"X"}, path{" "}, space{"."}, objective{"O"}, spacing{spacing}, arrows{arrows} {
		}
		 ...
		 */
		ConsoleView::ConsoleView(const ConsoleView &vue) : wall{vue.wall}, path{vue.path}, space{vue.space}, objective{vue.objective}, spacing{vue.spacing}, arrows{vue.arrows} {
		}
		...
		void ConsoleView::printPlayerInfos(const model::Player *const player) const {
			...
			this->lineBreak();
			this->printMessage("Players infos :\n"
							   "===============\n"
		...
		void ConsoleView::printInstructions(void) const {
			this->lineBreak();
			this->lineBreak();
			this->printMessage("Regles :\n"
							   "========");
			this->printMessage("  Dans un labyrinthe enchante, les joueurs partent a la chasse aux objets et aux creatures magiques. Chacun cherche a se frayer un chemin jusqu'a eux en faisant coulisser astucieusement les couloirs.\n"
			...
			this->lineBreak();
			this->printMessage("Deroulement :\n"
							   "=============");
			this->printMessage("  Chaque joueur se voit attribue differents objectifs a aller chercher dans le labyrinthe.\n"
			...
			this->lineBreak();
			this->printMessage("Modification du labyrinthe :\n"
							   "============================");
			this->printMessage("  12 fleches sont dessinees en bordure de plateau.\n"
			...
			this->lineBreak();
			this->printMessage("Deplacement du pion :\n"
							   "=====================");
			this->printMessage("  Des qu'il a modifie le labyrinthe, le joueur peut deplacer son pion. Il peut le deplacer aussi loin qu'il veut jusqu'a n'importe quelle plaque en suivant un couloir ininterrompu.\n"
			...
			this->lineBreak();
			this->printMessage("Fin du jeu :\n"
							   "============");
			this->printMessage("  La partie s'arrete des qu'un joueur a atteint tous ses objectifs et qu'il est revenu a son point de depart.");
		}
		...
		void ConsoleView::printBoardObjectives(model::Board *board, std::vector<model::Tile *> availableTiles) const {
			...
			if (std::any_of(availableTiles.begin(), availableTiles.end(), [](model::Tile *tile) {
				return tile->hasObjective();
			})) {
				this->lineBreak();
		...
		void ConsoleView::printPlayersInfos(std::vector<model::Player *> players) const {
			...
			for (model::Player *const &player : players) {
				this->printMessage("  " + player->getNickname() + " :\t" + ((player->getNickname().length() <= 3) ? "\t" : "") + player->getPosition().toString() + "\t>\t" + player->getCurrentObjective()->getObjective());
			}
		}
