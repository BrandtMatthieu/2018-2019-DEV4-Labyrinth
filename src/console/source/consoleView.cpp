...
namespace Labyrinth_44422 {
    namespace console {
         ...
         */
        void
        ConsoleView::printTileLine(const Labyrinth_44422::model::Tile *const tile, const unsigned int &ligneDansTuile,
            ...
            for (unsigned int caractereDeTuile = 0; caractereDeTuile < this->tileSize; caractereDeTuile++) {
                if (ligneDansTuile < this->wall1) {
                    ...
                    if (caractereDeTuile < this->wall1) {
                        ...
                        } else if (tile->hasObjective() && tile->getObjective().length() >= 3 &&
                                   std::abs(static_cast<int>(caractereDeTuile - this->tileSize / 2)) <= 1 &&
                                   ligneDansTuile == this->tileSize / 2) {
                            str += tile->getObjective().at(1 + (caractereDeTuile - this->tileSize / 2));
                        } else if (tile->hasObjective() && tile->getObjective().length() >= 1 &&
                                   caractereDeTuile == this->tileSize / 2 && ligneDansTuile == this->tileSize / 2 &&
                                   tile->hasStartNumber()) {
                            str += tile->getObjective().at(0);
                        } else if (tile->hasObjective() && caractereDeTuile == this->tileSize / 2 &&
                                   ligneDansTuile == this->tileSize / 2 && tile->hasStartNumber()) {
                            str += "?";
                        } else {
                            str += path;
                        }
        ...
        void ConsoleView::printBoard(const model::Board *const board) const {
            ...
            if (this->arrows) {
                ...
                for (unsigned int tuileDansRangee = 0; tuileDansRangee < board->getMaxSizeX(); tuileDansRangee++) {
                    for (unsigned int caractereDeTuile = 0; caractereDeTuile < this->tileSize; caractereDeTuile++) {
                        if (caractereDeTuile == this->tileSize / 2 &&
                            board->canInsertTile(model::Position{tuileDansRangee, 0}, model::InsertSide::DOWN)) {
                            str += "v"; // v ↓
                        } else {
                            str += " ";
                        }
            ...
            for (unsigned int rangeeDeTuile = 0;
                 rangeeDeTuile < board->getMaxSizeY(); rangeeDeTuile++) { // chaque ligne de tuile
                for (unsigned int ligneDansTuile = 0;
                     ligneDansTuile < this->tileSize; ligneDansTuile++) { // ligne horizontale dans tuile
                    str = "";
                    if (this->arrows && ligneDansTuile == this->tileSize / 2 &&
                        board->canInsertTile(model::Position{0, rangeeDeTuile}, model::InsertSide::RIGHT)) {
                        str += "> "; // →
                    } else if (this->arrows) {
                        str += "  ";
                    }
                    for (unsigned int tuileDansRangee = 0; tuileDansRangee < board->getMaxSizeX(); tuileDansRangee++) {
                        this->printTileLine(board->getTilesAt(rangeeDeTuile * board->getMaxSizeX() + tuileDansRangee),
                                            ligneDansTuile, str);
        ...
        void ConsoleView::printPlayerInfos(const model::Player *const player) const {
            ...
            this->printMessage(
                    ...
                    player->getPosition().toString() + "\n"
                                                       "  Current objective :\t\t" +
                    player->getCurrentObjective()->getObjective() + "\n"
                                                                    "  Objectives left count :\t" +
                    std::to_string(player->getObjectiveCardsLeftCount()) + "\n"
                                                                           "  Completed objectives count :\t" +
                    std::to_string(player->getCompletedObjectiveCardsCount()));
        }
        ...
        void ConsoleView::printHelp(const model::Board *const board) const {
            ...
            this->printMessage(
                    ...
                    "\n"
                    "> goto <x> <y>\n"
                    "  moves your pawn to a location, between (1;1) (upper left corner) and " +
                    model::Position(board->getMaxSizeX() - 1, board->getMaxSizeY() - 1).toString() +
                    " (bottom right corner)\n"
            ...
            for (const auto &tile : board->getTiles()) {
                if (tile->hasObjective()) {
                    this->printMessage("  " + tile->getObjective() + " :" +
                                       std::string(1 + ((15 - tile->getObjective().length()) / 4), '\t') +
                                       tile->getPosition().toString());
                }
            }
            if (std::any_of(availableTiles.begin(), availableTiles.end(),
                            [](model::Tile *tile) { return tile->hasObjective(); })) {
                this->lineBreak();
        ...
        void ConsoleView::printPlayersInfos(std::vector<model::Player *> players) const {
            ...
            for (model::Player *const &player : players) {
                this->printMessage(
                        "  " + player->getNickname() + " :\t" + ((player->getNickname().length() <= 3) ? "\t" : "")
