        ...
        void ControllerConsole::start(void) {
            ...
            while (!this->game->hasWinner()) {
                ...
                while (true) {
                    ...
                    if (std::regex_match(answer, std::regex(" {0,}(help|h) {0,}"))) {
                    ...
                    } else if (std::regex_match(answer, std::regex(
                            " {0,}(insert|i) {1,}[0-9]+ {1,}(up|down|left|right) {0,}"))) {
                        if (this->game->getCurrentPlayer()->hasInsertedTile()) {
                            ...
                            this->consoleView->printError(
                                    "You have already inserted a tile. Please do another action.");
                            continue;
                        }
                        ...
                        if (sideStr == "up") {
                            ...
                            this->consoleView->printError(
                                    "Unknown insert side. Please use only [ UP | DOWN | RIGHT | LEFT ]");
                            continue;
                        }

                        std::smatch positionMatch;
                        std::regex_search(answer, positionMatch, std::regex("([0-9]+)"));
                        unsigned int position = static_cast<unsigned int>(
                                (std::stoi(positionMatch.str(0)) < 1 ? 1 : std::stoi(positionMatch.str(0))) - 1);
                        ...
                        switch (side) {
                            ...
                            case model::InsertSide::DOWN:
                                ...
                                this->consoleView->printBoard(this->game->getBoard());
                                this->consoleView->printAvailableTile(this->game->getAvailableTiles().at(0));
                                this->consoleView->printBoardObjectives(this->game->getBoard(),
                                                                        this->game->getAvailableTiles());
                                break;
                        ...
                        if (!this->game->getCurrentPlayer()->hasInsertedTile()) {
                            this->consoleView->printError(
                                    "You cannot move you pawn now since you haven't inserted a tile in the Labyrinth yet.");
                            this->consoleView->printError(
                        ...
                        unsigned int x = static_cast<unsigned int>(
                                (std::stoi(matchCoo.str(1)) < 1 ? 1 : std::stoi(matchCoo.str(1))) - 1);
                        unsigned int y = static_cast<unsigned int>(
                                (std::stoi(matchCoo.str(2)) < 1 ? 1 : std::stoi(matchCoo.str(2))) - 1);
                        ...
                        if (!this->game->getCurrentPlayer()->hasInsertedTile()) {
                            this->consoleView->printError(
                                    "You cannot end you turn without inserting a tile in the labyrinth.");
