...
namespace Labyrinth_44422 {
	namespace model {
		 ...
		 */
		Game::Game(const Game &game) : board{new Board(*game.board)}, currentPlayerIndex{game.currentPlayerIndex} {
		...
		void Game::addPlayer(const std::string &name) {
			if (this->getPlayersCount() >= this->getMaxPlayers()) {
				throw std::runtime_error("Error while adding player. Maximum amount of player (" + std::to_string(this->getMaxPlayers()) + ") already reached.");
			}
		...
		void Game::generateTiles(void) {
			...
			for (unsigned int i = 0; i < this->getBoard()->getUnmovableTilesPositions().size(); i++) {
				...
				if (this->getBoard()->getUnmovableTilesPositions().at(i).getX() < this->getBoard()->getMaxSizeX() / 2) {
					pathRIGHT = true;
					pathLEFT = this->getBoard()->getUnmovableTilesPositions().at(i).getX() == 2 && this->getBoard()->getUnmovableTilesPositions().at(i).getY() != 2;
				} else {
					pathLEFT = true;
					pathRIGHT = this->getBoard()->getUnmovableTilesPositions().at(i).getX() == this->getBoard()->getMaxSizeX() - 3 && this->getBoard()->getUnmovableTilesPositions().at(i).getY() != this->getBoard()->getMaxSizeY() - 3;
				}

				if (this->getBoard()->getUnmovableTilesPositions().at(i).getY() < this->getBoard()->getMaxSizeY() / 2) {
					pathDOWN = true;
					pathUP = this->getBoard()->getUnmovableTilesPositions().at(i).getX() != this->getBoard()->getMaxSizeX() - 3 && this->getBoard()->getUnmovableTilesPositions().at(i).getY() == 2;
				} else {
					pathUP = true;
					pathDOWN = this->getBoard()->getUnmovableTilesPositions().at(i).getX() != 2 && this->getBoard()->getUnmovableTilesPositions().at(i).getY() == this->getBoard()->getMaxSizeY() - 3;
				}

				this->getBoard()->setTile(this->getBoard()->getUnmovableTilesPositions().at(i), new Tile{pathUP, pathDOWN, pathRIGHT, pathLEFT, this->getBoard()->getUnmovableTilesPositions().at(i), false, this->getFirstGameObjective(), 0});
			}
		...
		void Game::currentPlayerCheckObjective(void) {
			...
			if (!this->getCurrentPlayer()->getObjectiveCardsLeft().empty()) {
				if (this->getCurrentPlayer()->getCurrentObjective()->getObjective() == this->getBoard()->getTilesAt(this->getCurrentPlayer()->getPosition())->getObjective()) {
					this->getCurrentPlayer()->completeCurrentObjective();
				}
			}

			/* Put his last objective card and is maybe winner */
			if (this->getCurrentPlayer()->getObjectiveCardsLeftCount() == 0 && this->getCurrentPlayer()->getPosition() == this->getBoard()->getPlayersDefaultPositions().at(this->currentPlayerIndex)) {
		...
		/**
		 * Makes the current player insert a tile in the board
		 * @param line the line where to insert the tile
		 * @param side the side the player inserts the tile in
		 * @throw runtime_error if there is no tile to insert
		 * @throw runtime_error if the tile to insert is null
		 */
