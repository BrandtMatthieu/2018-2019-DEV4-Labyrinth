...
namespace Labyrinth_44422 {
	namespace model {
		 ...
		 */
		Board::Board(const Position &maxPosition) : maxSize{maxPosition}, tiles{std::vector<Tile *>(maxPosition.getX() * maxPosition.getY())}, playersDefaultPositions{Position{0, maxSize.getY() - 1}, Position{maxSize.getX() - 1, maxSize.getY() - 1}, Position{maxSize.getX() - 1, 0}, Position{0, 0}} {
		}
		 ...
		 */
		Board::Board(const Board &board) : maxSize{board.getMaxSize()}, tiles{std::vector<Tile *>()} {
		...
		Tile *Board::getTilesAt(const Position &position) const {
			if (!this->isPositionInsideBoard(position)) {
				throw std::invalid_argument("Error while getting the tile at " + position.toString() + ". Position is out of board's bounds (1;1) to " + this->maxSize.toString());
			}
		...
		bool Board::isPositionInsideBoard(const Position &position) const {
			return position.getX() < this->maxSize.getX() && position.getY() < this->maxSize.getY();
		}
		...
		std::vector<Position> Board::getUnmovableTilesPositions(void) const {
			return std::vector<Position>{Position(0, 2), Position(0, 4), Position(2, 0), Position(2, 2), Position(2, 4), Position(2, 6), Position(4, 0), Position(4, 2), Position(4, 4), Position(4, 6), Position(6, 2), Position(6, 4)};
		}
