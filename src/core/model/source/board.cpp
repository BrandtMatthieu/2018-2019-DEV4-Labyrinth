...
namespace Labyrinth_44422 {
    namespace model {
        ...
        unsigned int Board::indexOf(const std::vector<Tile *> &myVector, const Tile *const tile) const {
            if (tile == nullptr) {
                throw std::invalid_argument(
                        "Error while initializing the board. Cannot get the index if tile is null.");
            }
            if (!this->includes(myVector, tile)) {
                throw std::runtime_error(
                        "Error while getting the index of the tile. The tile doesn't exists inside the vector.");
            }
        ...
        void Board::setTile(const Position &position, Tile *const tile) {
            if (!this->isPositionInsideBoard(position)) {
                throw std::invalid_argument(
                        "Error while setting a tile at a position. Position (position) is out of bounds");
            }
            if (tile == nullptr) {
                throw std::invalid_argument(
                        "Error while setting a tile. Cannot set the tile if the tile is null (position).");
            }
        ...
        void Board::setTile(const unsigned int x, const unsigned int y, Tile *const tile) {
            if (!this->isPositionInsideBoard(Position(x, y))) {
                throw std::invalid_argument(
                        "Error while setting a tile at a position. Position (x;y) is out of bounds");
            }
            if (tile == nullptr) {
                throw std::invalid_argument(
                        "Error while setting a tile. Cannot set the index if the tile is null (x;y).");
            }
        ...
        void Board::setTile(const unsigned int index, Tile *const tile) {
            if (index > tiles.size()) {
                throw std::invalid_argument(
                        "Error while setting a tile at a position. Position (index) is out of bounds");
            }
            if (tile == nullptr) {
                throw std::invalid_argument(
                        "Error while setting a tile. Cannot set the index if the tile is null (index).");
            }
        ...
        bool Board::canInsertTile(const Position &position, const InsertSide &side) const {
            ...
            switch (side) {
                ...
                case InsertSide::LEFT:
                    for (unsigned int i = position.getY() * this->getMaxSizeX();
                         i < (position.getY() + 1) * this->getMaxSizeX(); i++) {
                        lineMovable = lineMovable && this->tiles.at(i)->isMovable();
                    }
                    break;
            }
        ...
        bool Board::canGoToFrom(const Position &from, const Position &to) {
            ...
            while (!tilesToCheck.empty()) {
                ...
                if (currentTileX + 1 < this->maxSize.getX()) {
                    ...
                    if (currentTile->getPathRIGHT() && this->tiles[index]->getPathLEFT() &&
                        !this->includes(tilesPossible, this->tiles[index])) {
                        ...
                        if (this->includes(tilesNotPossible, this->tiles[index])) {
                            tilesNotPossible.erase(
                                    tilesNotPossible.begin() + this->indexOf(tilesNotPossible, this->tiles[index]));
                        }
                    } else {
                        if (!this->includes(tilesNotPossible, this->tiles[index])) {
                ...
                if (currentTileX >= 1) {
                    index = (currentTileX - 1) + currentTileY * this->maxSize.getX();
                    if (currentTile->getPathLEFT() && this->tiles[index]->getPathRIGHT() &&
                        !this->includes(tilesPossible, this->tiles[index])) {
                        if (!this->includes(tilesToCheck, this->tiles[index])) {
                            tilesToCheck.push_back(this->tiles[index]);
                        }
                ...
                if (currentTileY >= 1) {
                    ...
                    if (currentTile->getPathUP() && this->tiles[index]->getPathDOWN() &&
                        !this->includes(tilesPossible, this->tiles[index])) {
                        if (!this->includes(tilesToCheck, this->tiles[index])) {
                            tilesToCheck.push_back(this->tiles[index]);
                        }
                ...
                if (currentTileY + 1 < this->maxSize.getY()) {
                    index = currentTileX + (currentTileY + 1) * this->maxSize.getX();
                    if (currentTile->getPathDOWN() && this->tiles[index]->getPathUP() &&
                        !this->includes(tilesPossible, this->tiles[index])) {
                        if (!this->includes(tilesToCheck, this->tiles[index])) {
                            tilesToCheck.push_back(this->tiles[index]);
                        }
