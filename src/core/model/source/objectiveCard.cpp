...
namespace Labyrinth_44422 {
	namespace model {
		...
		bool ObjectiveCard::operator==(const ObjectiveCard & card) const {
			return this->hasObjective() && card.hasObjective()
			&& this->getObjective() == card.getObjective();
		}
		...
		bool ObjectiveCard::operator!=(const ObjectiveCard & card) const {
			return (this->hasObjective() && !card.hasObjective())
			|| (!this->hasObjective() && card.hasObjective())
			|| (this->hasObjective() && card.hasObjective() && this->getObjective() != card.getObjective());
		}
